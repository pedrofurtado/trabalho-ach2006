describe 'As a User' do
  ...
  context 'I can receive an email with reset password instructions' do
    scenario 'With correct email' do
      ...
      expect_sent_reset_password_instructions_email_for @user
    end
  end
  ...
  context 'I can reset password' do
    scenario 'With correct reset password token' do
  ...
  context 'I can not reset password' do
    scenario 'With wrong reset password token' do
  ...
  def expect_sent_reset_password_instructions_email_for(user)
    email_deliveries = ActionMailer::Base.deliveries

    expect(email_deliveries.count).to eq 1
    expect(email_deliveries.last.to.first).to eq user.email
  end
